...
- `uv venv` creates or refreshes the project-local virtual environment at
  `.venv`.
- `uv sync` installs the shared Python 3.12 dependencies currently tracked at
  the repo root: `capstone`, `pwntools`, `sympy`, and `z3-solver`.
- If extra Python packages or CLI tools are needed for challenge work, install
  them into the project-local `.venv` with `uv add`, `uv sync`, or
  `uv pip install` rather than using system-wide `python` or `pip`.
